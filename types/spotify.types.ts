...
// The audio analysis object is not yet in the Object Model at Spotify,
// therefore it is typed as any in this file.

// TrackObjects and AlbumObjects is specified in the docs as always having
// the available_markets property,
...
export interface SearchForItemParameterObject {
   ...
   * If `include_external=audio` is specified, the response will
 ...
 *
 * At the time of typing, the Audio Analysis Object is absent from the
 * Object Model, so it is typed as any.
...
export interface AlbumObjectSimplified extends ContextObject {
   ...
   * the original track is not available in the given market, and Spotify did not
   *  have any tracks to relink it with.
...
export interface FollowersObject {
  /**
   * A link to the Web API endpoint providing full details of the followers; `null`
   *  if not available.
   * Please note that this will always be set to `null`, as the Web API does not support
   *  it at the moment.
   */
  href: null;
  /**
...
export interface PlaylistBaseObject extends ContextObject {
  /**
   * Returns `true` if context is not search and the owner allows other users
...
export interface TrackObjectSimplified {
   ...
   *  did not have any tracks to relink it with.
   * The track response will still contain metadata for the original track, and a
   *  restrictions object containing the reason
...
export interface EpisodeObjectSimplified extends ContextObject {
   ...
   * @deprecated Note: This field is deprecated and might be removed
   * in the future. Please use the languages
   * field instead.
   */
  language: string;
  /**
   * A list of the languages used in the episode, identified by their [ISO 639](https://en.wikipedia.org/wiki/ISO_639)
   *  code.
   * Optional because sometimes only the deprecated
   *  language field is set and this one isn't set at all.
   */
